...
class AuthService
{
  ...
  public function createEmployeeByGovernment(array $data, int $governmentId, User $creator): User
  {
    $data['password'] = Hash::make($data['password'] ?? bcrypt('bshara is boombasti'));
    $data['role_id'] = 3;
    $data['is_verified'] = 1;
    $user = $this->users->create($data);
    $this->users->attachToGovernment($user, $governmentId);
    $this->log($creator->id, 'create_employee', [
      'employee_id' => $user->id,
  ...
  //   //   $message->to('bshara.h2003@gmail.com')
  //   //     ->subject('SMTP Test');
  //   // });
